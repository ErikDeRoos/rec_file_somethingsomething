...
## Testing Guidelines
- Tests that may operate on files should use an `IDisposable` wrapper that creates a temp folder under the test project bin, copies the needed fixture files, exposes working paths, and cleans them up on dispose for repeatable runs.
